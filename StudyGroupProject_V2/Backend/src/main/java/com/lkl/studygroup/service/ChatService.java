...
public class ChatService {
    ...
    public ListGroupChatMessagesResponse getGroupChatMessages(String groupId, String keyword, Integer pageNumber,
            Integer pageSize) {
    ...
    public DirectChat newDirectChat(String userId1, String userId2) {
        if (userId1.equals(userId2)) {
            throw new ExceptionResponse(ErrorCode.CANNOT_CHAT_WITH_YOURSELF);
        }
    ...
    public void sendDirectChatMessages(String userId, String chatId,
            DirectChatMessageRequest directChatMessageRequest) {
        Message message = new Message();
    ...
    public List<DirectChatDetailResponse> getDirectChatDetailByDirectChatId(String directChatId) {
        ...
        for (String userId : directChat.getParticipantIds()) {
            ...
            DirectChatDetailResponse directChatDetailResponse = new DirectChatDetailResponse(userId,
    ...
    public ListUserDirectChatsResponse getListUserDirectChatsResponse(String userId, String keyword, Integer pageNumber,
            Integer pageSize) {
        ...
        Pageable pageDetails = PageRequest.of(pageNumber - 1, pageSize);
        Page<DirectChat> result = directChatRepository.findDirectChatsBySenderId(userId, pageDetails, listUserIds,
                isSearched);
        ...
        List<String> receiverIds = result.getContent().stream().map(
                chat -> chat.getParticipantIds().stream().filter(id -> !id.equals(userId)).findFirst().orElse(userId))
        ...
        for (User user : receivers) {
            listReceiversInfo.put(user.getId().toString(), new ReceiverInfo(user.getId().toString(), user.getEmail(),
                    user.getAvatarUrl(), user.getLastName(), user.getFirstName()));
        }
    ...
    public ListDirectChatMessagesResponse getDirectChatMessages(String chatId, String keyword, Integer pageNumber,
            Integer pageSize) {
        Pageable pageDetails = PageRequest.of(pageNumber - 1, pageSize);
